...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  igtl::PointMessage::Pointer sender;
  sender = igtl::PointMessage::New();
  sender->SetDeviceName("NowySerwer");
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
	  ...
	  igtlFloat32 negativePosition[3];
	  for (int j = 0; j < 3; j++) {
		  negativePosition[j] = pos[j] * -1;
	  }

	  igtl::PointElement::Pointer negativePoint;
	  negativePoint = igtl::PointElement::New();
	  negativePoint->SetName(pointElement->GetName());
	  negativePoint->SetGroupName(pointElement->GetGroupName());
	  negativePoint->SetRGBA(rgba);
	  negativePoint->SetPosition(negativePosition);
	  negativePoint->SetRadius(pointElement->GetRadius());
	  negativePoint->SetOwner(pointElement->GetOwner());

	  sender->AddPointElement(negativePoint);
      }

	igtl::Sleep(5000);
	sender->Pack();
	socket->Send(sender->GetPackPointer(), sender->GetPackSize());
    }
